...
void print_paddr_and_flags(char *bigmem, int pagemap_file,
                           int kpageflags_file) {
    uint64_t paddr;
    uint64_t page_flags;
...
int main(int argc, char **argv) {
    ...
    mbind(from_page_base, pagesize * page_count, MPOL_BIND, &node0,
          sizeof(node0) * 8, 0);
    ...
    mbind(to_page_base, pagesize * page_count, MPOL_BIND, &node1,
          sizeof(node1) * 8, 0);
    ...
    printf("----------From Pages------------\n");
    for (i = 0; i < page_count; ++i) {
        print_paddr_and_flags(from_pages + pagesize * i, pagemap_fd,
                              kpageflags_fd);
    }
    printf("----------To Pages------------\n");
    for (i = 0; i < page_count; ++i) {
        print_paddr_and_flags(to_pages + pagesize * i, pagemap_fd,
                              kpageflags_fd);
    }
    ...
    /*rc = numa_move_pages(0, page_count, addr, nodes, status,
     * move_page_flag);*/
    rc =
        syscall(439, 0, page_count, from_addr, to_addr, status, move_page_flag);
    ...
    cpu_time_used = ( end.tv_sec - begin.tv_sec)
    ...
    printf("Total_cycles\tBegin_timestamp\tEnd_timestamp\n"
           "%f\t%f\t%f\n",
           cpu_time_used, (double)begin.tv_sec, (double)end.tv_sec);
    ...
    for (i = 0; i < page_count; i++) {
        if (from_pages[i * pagesize] != (char)i) {
            fprintf(stderr,
                    "*** From Page contents corrupted. expected: %d, got: %d\n",
    ...
    for (i = 0; i < page_count; i++) {
        if (to_pages[i * pagesize] != (char)(i + 1)) {
            fprintf(stderr,
                    "*** To Page contents corrupted. expected: %d, got: %d\n",
